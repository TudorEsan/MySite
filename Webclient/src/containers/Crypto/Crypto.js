    ...
    render() {
        if (this.state.crypto) {
            ...
            return (
                <>
                    {!!this.state.isOpen && (
                        <AddCryptoDialog isOpen={this.state.isOpen} setState={this.setState}/>
                    )}
                    {!!this.state.sellIsOpen && (
                        <SellCrypto isOpen={this.state.sellIsOpen} setState={this.setState}/>
                    )}
                    <SupremeContainer isOpen={this.state.isOpen || this.state.sellIsOpen}>
                    <Container>
                        <TitleContainer>
                    ...
                    <Container2>
                        <ButtonContainer>
                            <Button theme={theme}>
                                <ButtonLabel onClick={() => {this.openAddDialog()}}>Add Crypto</ButtonLabel>
                            </Button>
                                <Button theme={theme} onClick={() => { this.openSellDialog() }}>
                                <ButtonLabel>Sell</ButtonLabel> 
                            </Button>
                        </ButtonContainer>
                        <Card theme={theme}>
                            <Heading theme={theme} >Overview</Heading>
                            <Amount theme={theme} >{this.twoDigits(this.state.crypto.actualAmount)} USD</Amount>
                            <Percentage theme={theme} percentage={this.state.crypto.actualAmount - this.state.crypto.amountInvested}>{this.twoDigits(this.state.crypto.actualAmount - this.state.crypto.amountInvested)} USD {this.twoDigits(this.state.crypto.totalGrowth * 100)}%</Percentage>
                                ...
                                if (!['amountInvested', 'actualAmount', 'totalGrowth'].includes(key)) {
                                    return (
                                        <CryptoContainer key={index}>
                                            <CryptoImg src={this.state.crypto[key].icon} />
                                            <CryptoName theme={theme}>{this.state.crypto[key].name}</CryptoName>
                                            <Divider></Divider>
                                            <PriceContainer>
                                                <CryptoPrice theme={theme}>{this.twoDigits(this.state.crypto[key].currentUsd)}$</CryptoPrice>
                                                <SmallPercentage theme={theme} percentage={this.state.crypto[key].growth}>{ this.twoDigits(Math.abs(this.state.crypto[key].growth * 100))}%</SmallPercentage>
                                            </PriceContainer>
                                        </CryptoContainer>
                                    )
                                }
                        ...
                        <Card theme={theme}>
                            ...
                            {Object.keys(this.state.crypto).map((key, index) => {
                                if (!['amountInvested', 'actualAmount', 'totalGrowth'].includes(key)) {
                                    return (
                                        this.state.crypto[key].transactions.map((elem, index) => {
                                            if (elem.type !== "sold") {
                                                return (
                                                    <CryptoContainer key={index}>
                                                        <CryptoImg src={this.state.crypto[key].icon} />
